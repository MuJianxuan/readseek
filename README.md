...
flag to analyze image contents with the SmolVLM-500M vision model:
...
(~0.4 GB GGUF + ~0.2 GB multimodal projection) are downloaded lazily into the
...
readseek downloads the SmolVLM-500M vision model (`ggml-org/SmolVLM-500M-Instruct-GGUF`,
quantized from `HuggingFaceTB/SmolVLM-500M-Instruct`) into the user cache directory on
