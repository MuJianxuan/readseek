...
mod tests {
    ...
    #[test]
    fn load_index_rejects_out_of_range_path_entries() {
        let readseek_dir = std::env::temp_dir().join(format!(
            "readseek-index-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
        ...
        shard.insert("needle".to_owned(), vec![PathBuf::from("file.rs")]);
        let mut data = serialize_shard(&shard).unwrap();
        let first_path = INDEX_HEADER_SIZE + offset_of!(IndexNameEntry, first_path);
        data[first_path..first_path + size_of::<U32<LittleEndian>>()]
            .copy_from_slice(&1_u32.to_le_bytes());
        let crc32 = crc::Crc::<u32>::new(&CRC_32_ISO_HDLC);
        let checksum = crc32.checksum(&data[INDEX_HEADER_SIZE..]);
        data[INDEX_CHECKSUM_OFFSET..INDEX_CHECKSUM_OFFSET + size_of::<U32<LittleEndian>>()]
            .copy_from_slice(&checksum.to_le_bytes());

        let path = def_index_shard_path(&readseek_dir, "needle");
        fs::create_dir_all(path.parent().unwrap()).unwrap();
        fs::write(path, data).unwrap();

        assert_eq!(load_index(&readseek_dir, "needle").unwrap(), None);
        fs::remove_dir_all(readseek_dir).unwrap();
    }
}
