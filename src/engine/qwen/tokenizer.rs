...
impl Tokenizer {
    ...
    pub(crate) fn from_gguf(gguf: &Gguf) -> Result<Self> {
        ...
        let (token_pieces, special_tokens) =
            build_token_pieces_and_specials(&token_texts, &token_types, &byte_chars)?;
        ...
        let merges = build_merges(merge_texts, &token_ids)?;
...
type TokenPieces = (Vec<Vec<u8>>, Vec<(String, u32)>);

fn build_token_pieces_and_specials(
    texts: &[String],
    types: &[u32],
    byte_chars: &[char; 256],
) -> Result<TokenPieces> {
    let byte_values: HashMap<_, _> = byte_chars
        ...
        .collect::<Result<_>>()?;
    let mut token_pieces = Vec::with_capacity(texts.len());
    let mut special_tokens = Vec::new();
    for (index, (text, token_type)) in texts.iter().zip(types).enumerate() {
    ...
    Ok((token_pieces, special_tokens))
}
