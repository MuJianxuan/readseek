...
mod tests {
    ...
    fn load_maps_a_valid_gguf() {
        ...
        bytes.resize(bytes.len().next_multiple_of(DEFAULT_ALIGNMENT), 0);

        let timestamp = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system time must be after the Unix epoch")
            .as_nanos();
        let path = std::env::temp_dir().join(format!("readseek-gguf-{timestamp}"));
        std::fs::write(&path, bytes).expect("write GGUF fixture");

        let gguf = Gguf::load(&path).expect("map and parse GGUF fixture");
        assert_eq!(gguf.architecture(), "qwen2");
        drop(gguf);
